...
from config import BATCH_SIZE, VECTOR_COLUMN
from utils.db_connection import init_db, get_pool, close_db
from utils.clients import hf_client  # Imported the new Hugging Face TEI client
...
async def process_batch(pool, rows, target_column: str):
    """Fetches embeddings from the new Hugging Face TEI endpoint and updates the database."""
    chunk_ids = [row["id"] for row in rows]
    texts = [row["chunk_text"] for row in rows]

    try:
        response = await hf_client.feature_extraction(text=texts)
        vectors = response if isinstance(response, list) else response.tolist()
        ...
        print(f"\n❌ Error processing batch via Hugging Face Endpoint: {e}")
        raise e
