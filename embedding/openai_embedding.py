...
async def process_batch(pool, rows, target_column: str):
    """Fetches 3072-dimension text embeddings from OpenAI API and updates the database."""
    chunk_ids = [row["id"] for row in rows]
    texts = [row["chunk_text"] for row in rows]

    try:
        # 1. Native async call to OpenAI API with specified 3072 dimensions
        ...
        # 2. Extract vectors from OpenAI response structure
        update_data = []
...
async def main():
    ...
    if total_to_process == 0:
        print("✅ All OpenAI embeddings are already generated!")
        await close_db()
        return
    ...
    pbar = tqdm(
        ...
        desc="🚀 Generating OpenAI vectors",
        unit="chunk",
        ncols=100,
    )
    ...
    pbar.close()
    print("🏁 OpenAI embedding generation process completed!")
    await close_db()
